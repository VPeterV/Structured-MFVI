...
class VariationalLSTM(nn.Module):
    r"""
    from supar https://github.com/yzhangcs/parser/blob/8ed9ccb8e542655fd6fd1b6f7faaf084d13a866e/supar/modules/lstm.py#L84
    ...
    def __init__(self, conf, input_size):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = conf.n_lstm_hidden
        self.num_layers = conf.n_lstm_layers
        self.bidirectional = conf.bilstm
        self.dropout = conf.lstm_dropout
        ...
        for _ in range(self.num_layers):
            self.f_cells.append(nn.LSTMCell(input_size=input_size, hidden_size=self.hidden_size))
            if self.bidirectional:
                self.b_cells.append(nn.LSTMCell(input_size=input_size, hidden_size=self.hidden_size))
