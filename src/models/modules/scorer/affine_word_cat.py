...
class SRLFirstOrderScorer(nn.Module):
    def __init__(self, conf, input_dim, n_span_labels, ensemble_label = False):
        ...
        if not self.cat_rel_arc:
            ...
            self.ph_att = nn.Linear(biaffine_dim * 2, arc_n_out)
            self.pt_att = nn.Linear(biaffine_dim * 2, arc_n_out)
        
        if not self.cat_rel_label:
            ...
            # self.ph_labels_att = Biaffine(biaffine_label_dim, n_out = n_span_labels, init = conf.label_init)
            self.ph_labels_att = nn.Linear(biaffine_label_dim * 2, n_span_labels)

            if ensemble_label:
                self.t_label_mlp = MLP(input_dim, biaffine_label_dim)  # heads labels
                # self.pt_labels_att = Biaffine(biaffine_label_dim, n_out = n_span_labels, init = conf.label_init)
                self.pt_labels_att = nn.Linear(biaffine_label_dim * 2, n_span_labels)
    ...
    def forward(self, word):
        ...
        if not self.cat_rel_arc:
            ...
            p = p[:, :, None, :].expand(-1, -1, seqlen, -1)
            h = h[:, None, :, :].expand(-1, seqlen, -1, -1)
            t = t[:, None, :, :].expand(-1, seqlen, -1, -1)
            
            ph_cat = torch.cat([p, h], dim = -1)
            ph = self.ph_att(ph_cat)
            
            pt_cat = torch.cat([p, t], dim = -1)
            pt = self.pt_att(pt_cat)
        ...
        def get_t_label():
            if not self.cat_rel_label:
                ...
                t_label = self.t_label_mlp(word)
                t_label = t_label[:, None, :, :].expand(-1, seqlen, -1, -1)
        ...
        if not self.cat_rel_label:
            ...
            p_label = p_label[:, :, None, :].expand(-1, -1, seqlen, -1)
            h_label = h_label[:, None, :, :].expand(-1, seqlen, -1, -1)
            
            ph_label = self.ph_labels_att(p_label, h_label)
            
     
        pt_label = get_t_label()
