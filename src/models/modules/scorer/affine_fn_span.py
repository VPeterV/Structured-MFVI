...
def define_concat(input_dim, n_out):
    # l_mlp = MLP(input_dim, biaffine_dim)
    # r_mlp = MLP(input_dim, biaffine_dim)    
    # biaffine = Biaffine(biaffine_dim, n_out = n_out, init = init)
...
class FirstOrderScorer(nn.Module):
    def __init__(self, conf, input_dim, n_span_labels: List):
        ...
        elif scoring_type == 'concat':
            # spans scorers
            self.span_linear = define_concat(input_dim, span_n_out)
            
            # arcs scorers
            self.h2h_linear = define_concat(input_dim, arc_n_out)
            self.t2t_linear = define_concat(input_dim, arc_n_out)
        else:
    ...
    def concat_pipeline(self, x, linear, dropout):
        ...
        x_right = x.unsqueeze(-2).expand(-1, -1, -1, seqlen)
