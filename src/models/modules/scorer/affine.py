...
class SRLFirstOrderScorer(nn.Module):
    def __init__(self, conf, input_dim, n_span_labels):
        ...
        # label scorers
        self.sh_label_mlp = MLP(input_dim, biaffine_label_dim)  # span heads labels
        self.st_label_mlp = MLP(input_dim, biaffine_label_dim)  # span tails labels
    ...
    def forward(self, x):
        ...
        # labels
        sh_label = self.sh_label_mlp(x)
        st_label = self.st_label_mlp(x)
        
        sh_label = self.label_dropout(sh_label)
        st_label = self.label_dropout(st_label)
        
        span_label = self.span_labels_att(sh_label, st_label)
