...
class SecondOrderMF(nn.Module):
    ...
    @torch.enable_grad()
    def forward(self, foscores, soscores, maskspan, maskarc, mask2o_pspan, mask2o_psib, mask2o_pcop, inference = False):
        ...
        if self.structured:
            if self.mf_split:
                ...
                span_logits, ph_logits, pt_logits = self.structured_mfvi(foscores, soscores, maskspan, maskarc, mask2o_pspan, mask2o_psib, mask2o_pcop)
        else:
            span_logits, ph_logits, pt_logits = self.mfvi(foscores, soscores, maskspan, maskarc, mask2o_pspan, mask2o_psib, mask2o_pcop)

        results = {'logits':[span_logits, ph_logits, pt_logits]}
        
        return results
        
    def mfvi(self, foscores, soscores, maskspan, maskarc, mask2o_pspan, mask2o_psib, mask2o_pcop):
        ...
        if 'p' in self.score_types:
            span_psh = span_psh * mask2o_pspan
            span_pst = span_pst * mask2o_pspan
        else:
            span_psh = span_psh * zero
            span_pst = span_pst * zero
        ...
        if 's' in self.score_types:
            ...
            # ph_cop = ph_cop * mask2o_pps
            # pt_cop = pt_cop * mask2o_pps
    ...
    def structured_mfvi(self, foscores, soscores, maskspan, maskarc,  mask2o_pspan, mask2o_psib, mask2o_pcop):
        ...
        for _ in range(self.max_iter):
            ...
            tmp_span = span[..., 1] + (q_ph.unsqueeze(3) * span_psh + q_pt.unsqueeze(2) * span_pst).sum(1)
            tmp_ph = ph + (q_span_1.unsqueeze(1) * span_psh + q_ph.unsqueeze(2) * ph_sib).sum(3) + (q_ph.unsqueeze(1) * ph_cop).sum(2)
            tmp_pt = pt + (q_pt.unsqueeze(2) * pt_sib).sum(3) + (q_span_1.unsqueeze(1) * span_pst + q_pt.unsqueeze(1) * pt_cop).sum(2)
