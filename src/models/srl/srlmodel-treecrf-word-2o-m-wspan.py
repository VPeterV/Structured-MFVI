...
from .utils import unfold_span_label, potential_norm, unfold_graphs
...
class SRLModel(nn.Module):
    def __init__(self, conf, fields):
        ...
        self.spanencoder = SpanEncoder(conf.encoder, self.affine_input)
    ...
    def forward(self, x_table, y_table,  inference = False):
        ...
        if inference:
            ...
            span_preds, span_label_scores, span_arc_scores,\
                span_m, ph_m, pt_m = self.inference(h, mf, maskspan, self.conf.decoder.mbr)
            ...
            span_repr = self.spanencoder(h, gold_spans)
    ...
    def inference(self, hiddens, mf, maskspan, mbr = True):
        ...
        # breakpoint()
        span_repr = self.spanencoder(hiddens, pred_spans)
